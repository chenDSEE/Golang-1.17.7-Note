...
func readRequest(b *bufio.Reader) (req *Request, err error) {
	...
	/* 所有 Header 的 key-value pair 都会发生内存的深拷贝。而不是直接指向 tp 的 buffer 里面
	 * 不是不能理解，因为不拷贝会有下面两个问题：
	 * 1. request 数据很多，而 bufio 用的又是环形缓冲区，不拷贝走，迟早会被覆盖掉的
	 * 2. 仅仅考虑 HTTP 1.0 的 request-respond 模式，确实可以不拷贝，直接指向 tp 的 buffer
	 *    但是 pipeline 启用了呢？HTTP 2.0 多路复用呢？
	 *
	...
	// NOTE: 另一方面，内部存在 bufio --> map 的内存拷贝
