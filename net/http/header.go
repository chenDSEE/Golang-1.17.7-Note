...
func (h Header) Clone() Header {
	...
	/* 为什么要先把长度算出来？节省 slice 扩容带来的消耗
	 * len(sv) 就是 h 全部 value 的总长度
	 * 然后新生成的 h2 所有 value 都是针对 sv 的一个 slice
	 * 所以 h2 实际的数据大部分都是在同一块内存上的
	 * 减少内存碎片，降低 GC 压力。
	 * 而且返回的 h2 到时候也是整个遍历的，有利于 cahce-line hit rate
	 */
