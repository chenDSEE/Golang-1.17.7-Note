...
func readTransfer(msg interface{}, r *bufio.Reader) (err error) {
	...
	// else:
	// case 1: 没有 Transfer-Encoding， 非 chunked, t.parseTransferEncoding() 返回 nil
	// case 2: chunked, t.parseTransferEncoding() 返回 nil，并且成功设置 chunked 模式

	// case 1: 定长 request 返回相应长度
	// case 2: 不定长 request 返回 -1, nil (chunked mode)
	// case 3: method RFC 规定没有 body
	...
	// 为不同种类的 body 这是不同的 Reader，并且最终挂载到 request.body 或者是 response.body
	switch {
	case t.Chunked:
		if noResponseBodyExpected(t.RequestMethod) || !bodyAllowedForStatus(t.StatusCode) {
			t.Body = NoBody
		} else {
			// case 1: chunked 方式，不定长 body
	...
	case realLength == 0:
		...
		t.Body = NoBody
	case realLength > 0:
		// case 3: 定长 body，采用 io.LimitReader 确保不会发生数据异常
...
func (b *body) readLocked(p []byte) (n int, err error) {
	if b.sawEOF {
		// 定长 body 已经读取完毕，不能再往下 read 了
		return 0, io.EOF
	}
	// 1. 先做了 LimitReader
	// 2. 然后是 Bufio.Reader(http.conn.bufr)
	// 3. 再然后是 http.connReader.Read() (http.conn.r)
	// 3.1 这时候，就会在 connReader.mutex 的保护下，进行 http.conn.rwc.Read()
	// 要是只 read 一点点（http.Request.Body.Read(outBuf) 的 len(buf) < len(bufio.buf)），这时候 p 就是 bufio.buf
	// 要是 read 很多（http.Request.Body.Read(outBuf) 的 len(buf) > len(bufio.buf)），这时候 p 就是 outBuf
	n, err = b.src.Read(p)
	...
	// 定长 body 通过下面的 LimitedReader 来确定这个 request 的 body 是不是已经读取完了
...
func (b *body) Close() error {
	...
	default:
		...
		// the trailer headers after the body, if present.
		_, err = io.Copy(io.Discard, bodyLocked{b}) // 这个 request 已经结束了，没用完的 body 可以直接扔了。避免影响下一次 requset 的 read
	}
	b.closed = true
	return err
}
