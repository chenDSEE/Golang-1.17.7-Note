...
type conn struct {
	...
	// bufr reads from r. 耗掉 4KB 内存
	bufr *bufio.Reader

	// bufw writes to checkConnErrorWriter{c}, which populates werr on error. 耗掉 4KB 内存
	bufw *bufio.Writer
...
func (c *conn) readRequest(ctx context.Context) (w *response, err error) {
	...
	/* c.rwc 是普通的 TCPConn 或 TLSConn，所以使用的是:
...
func (cw *chunkWriter) writeHeader(p []byte) {
	...
	// Trailer Header 在 RFC 上跟部分 Header 是冲突的
...
func (w *response) finishRequest() {
	...
	if !w.wroteHeader {
		// StatusOK: status line 返回 HTTP 200 OK
		w.WriteHeader(StatusOK)
	}

	/* 把 header + body flush 出去 */
	// response.w 是裹了 buffer 的 response.cw
	// 所以最后是通过 chunkWriter.Write() 把数据刷去 w.conn.bufw 的 buffer 里面
	// 所以 Header 的数据生成是在 chunkWriter.writeHeader() 里面
...
func (c *conn) setState(nc net.Conn, state ConnState, runHook bool) {
	...
	case StateHijacked, StateClosed:
		...
		// 所以 http.server 不需要再 track 这个 conn
...
func (c *conn) serve(ctx context.Context) {
	...
	for {
		...
		// net.http.response 将会被限制为 net.http.ResponseWriter interface，限制、规范其写入数据的方式
		// 至于 net.http.Request 嘛，确实限制一下，让它成为一个 Reader 会更好
		// c.server 在 srv.newConn(rw) 里面就设置好了
		/* 整个调用链：
		 * --> c.server 构造 http.serverHandler
		 * --> http.serverHandler.ServeHTTP()
		 * --> callbackFunction := http.ServeMux.Handler(r)
...
type Server struct {
	...
	mu         sync.Mutex
	// 一个 server 可以挂载很多个 listener，监听多个地址
...
func (srv *Server) Serve(l net.Listener) error {
	...
	for {
		...
		tempDelay = 0
		c := srv.newConn(rw) // 将 net.Conn(也就是 net.TCPConn 或 TLSConn) 封装为 net.http.conn
