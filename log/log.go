...
//
// 通过 log.Printf 将会自动换行，即使你忘记追加 \n 了
// Every log message is output on a separate line: if the message being
// printed does not end in a newline, the logger will add one.
//
// log.Panic() 将会调用 panic()，勉强可以恢复
// log.Fatal() 将会调用 os.Exit(1)，无法恢复
...
type Logger struct {
	mu     sync.Mutex // ensures atomic writes; protects the following fields, 并发安全
	prefix string     // prefix on each line to identify the logger (but see Lmsgprefix)，perfix 是配置后固定的，不需要每次都生成
	flag   int        // properties
	out    io.Writer  // destination for output, 通过 io.Writer 进行解耦，让 Logger 既能够向 socket 输出，也能够向文件输出
...
var std = New(os.Stderr, "", LstdFlags) // 内置的 logger
